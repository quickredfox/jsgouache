...
APP_DIST_ALT_DIR = File.join(APP_DIST_DIR,'alt')
...
task :dist do
  ...
  Dir.chdir(APP_DIST_DIR) do
    FileUtils.copy_file APP_FILE_NAME, File.join(APP_DIST_ALT_DIR,"#{APP_NAME}-#{APP_VERSION}.js")
    `cat #{APP_FILE_NAME} | ruby ../lib/jsmin.rb > #{File.join(APP_DIST_ALT_DIR,"#{APP_NAME}-#{APP_VERSION}-minified.js")}`
  end
